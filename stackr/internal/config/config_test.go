...
func TestStackDeploymentDefaults(t *testing.T) {
	cfg := Config{}
	meta := cfg.StackDeployment("mx5parts")
	require.Equal(t, "MX5PARTS_IMAGE_TAG", meta.TagEnv)
	require.Equal(t, []string{"mx5parts", "update"}, meta.Args)
}

func TestStackDeploymentOverride(t *testing.T) {
	cfg := Config{
		Global: GlobalConfig{
			Deploy: map[string]StackDeploy{
				"stacka": {
					TagEnv: "STACKA_TAG",
					Args:   []string{"stacka", "deploy"},
				},
			},
		},
	}

	meta := cfg.StackDeployment("stacka")
	require.Equal(t, "STACKA_TAG", meta.TagEnv)
	require.Equal(t, []string{"stacka", "deploy"}, meta.Args)
}
